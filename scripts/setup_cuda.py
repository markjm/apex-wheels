...
that lives in this repository.  Supports two install methods:

  network  - adds NVIDIA's apt repo and installs the cuda-toolkit package
             (fast, preferred on Debian/Ubuntu GitHub runners).
  local    - downloads the .run installer from NVIDIA and runs it silently
             (works everywhere, slower).

Usage:
    python scripts/setup_cuda.py <version> [--method auto|network|local]
